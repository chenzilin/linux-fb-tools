...
{
	dst[fb->vi.red.offset/8] = color->r;
	dst[fb->vi.green.offset/8] = color->g;
	dst[fb->vi.blue.offset/8] = color->b;
...
static int fb_write_argb32(FBInfo* fb, unsigned char* dst, Color* color)
{
	...
	dst[fb->vi.green.offset/8] = color->g;
	dst[fb->vi.red.offset/8] = color->r;
	dst[fb->vi.transp.offset/8] = color->a;
...
#else
			png_byte red = trans_values->red & 0xff;
			png_byte green = trans_values->green & 0xff;
			png_byte blue = trans_values->blue & 0xff;
#endif
			for(y = 0; y < h; y++)
			{
				src = row_pointers[y];
...
int main(int argc, char* argv[])
{
	...
	if(argc != 3)
	{
		printf("\nUsage: %s [jpg|png file] [framebuffer dev]\n", argv[0]);
